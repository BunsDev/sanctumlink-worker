...
export class StoreAttribute extends OpenAPIRoute {
  static readonly schema: OpenAPIRouteSchema = {
    tags: ["Identity", "Attribute"],
    summary: "Add an Attribute to an Identity",
    parameters: {
      uid: Path(String, {
  ...
  async handle(
    ...
    const { type: attributeType, value: attributeValue, valueHash: attributeHash, signature } = data.body;
    console.log({ identityId, attributeType, attributeValue, attributeHash, signature });
